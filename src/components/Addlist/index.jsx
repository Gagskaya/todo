...
export const AddList = ({ colors, onAddList }) => {
    ...
    const addList = () => {
        ...
        axios.post('http://localhost:3001/lists', {
            ...
            const color = colors.find(color => selectedColor === color.id);
            const newList = {
                ...data,
                color
            }
        ...
        <div className="todo__addlist">
            ...
            {visiblePopup && <div className="todo__addlist-popup">
                ...
                <ul className="todo__addlist-popup-colors">
                    {
                        colors.map(color => <li onClick={() => selectColor(color.id)} key={color.id}><Badge className={selectedColor === color.id && 'active'} color={color.hex} /></li>)
                    }
                </ul>
                <button className="btn" onClick={addList}>{isLoading ? "Добавление..." : "Добавить"}</button>
                <i className="close-btn" onClick={onClosePopup}>
