...
export const List = ({ items, onClick, isRemovable,onRemove }) => {
    const removeList = (item) => {
        if (window.confirm("Вы действильно хотите удалить список?")) {
            onRemove(item)
        }
    }
    return (
        <ul className="list">
            {items.map((item, index) =>
                <li key={index} className={item.active ? 'active' : ''} onClick={onClick}>
                    <i>{item.icon ? item.icon : <Badge color={item.color} />}</i>
                    <span>{item.name}</span>
                    {isRemovable && <i className="list-remove" onClick={()=> removeList(item)}>
